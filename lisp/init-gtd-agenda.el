;;; init-gtd-agenda.el ---  -*- lexical-binding: t -*-
;;; Commentary:
;;; Code:
...
(with-eval-after-load 'org-agenda
  (setq org-agenda-window-setup 'only-window
        ;; not show blockde tasks in agenda
                            ...
                            (org-agenda-skip-function
                             (lambda ()
                               (or (org-agenda/skip-entry)
                                   ...
                                   (org-agenda/skip-if-previous-sibling-not-done))))
...
(defun org-agenda/skip-if-previous-sibling-not-done ()
  "若前一个兄弟节点存在且状态不是DONE, 则跳过当前任务。"
  (let* ((has-prev-sibling nil)
         (prev-status (save-excursion
                        (save-restriction
                          (widen)
                          (when (org-goto-sibling t)
                            (setq has-prev-sibling t)
                            (org-get-todo-state))))))
    ;; 当存在上一个兄弟节点，且该状态不在 `org-done-keywords` 中时跳过
    (when (and has-prev-sibling
               (not (member prev-status org-done-keywords)))
